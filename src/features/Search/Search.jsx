...
import { useGetSearchResultQuery } from "../../shared/Api/movieApi"; 
import SearchIcon from "../../shared/assets/icon/SearchIcon";
        ...
        <div ref={searchRef}>
            <div className="relative z-50">
                <input
                    ...
                    className="bg-white px-[36px] py-2 rounded-lg w-[1000px] text-black"
                />
                <button aria-label="search" onClick={handleSearch} className="absolute left-3 top-[11px]">
                <SearchIcon />
                  </button>
            </div>

            <div className="flex flex-col absolute max-h-[400px] overflow-auto top-[75px] w-full max-w-[1000px] rounded-lg">
                {error && <p className="bg-[#212121] py-6 px-3 text-white">{t("error")} : {error.message}</p>}
                {isLoading ? (
                    <p className="bg-[#212121] py-6 px-3 text-white">{t("loading")}</p>
